...
def main(args):
    ...
    all_metrics = []
    block_hammer_beat_matrix = []
    block_handover_matrix = []
    blocks_stack_easy_matrix = []
    instructions = ["beat the block with the hammer", "handover the blocks", "stack blocks"]
    task2matrix = {"beat the block with the hammer": block_hammer_beat_matrix, 
             "handover the blocks": block_handover_matrix, 
             "stack blocks": blocks_stack_easy_matrix}
    ...
    for sample_dir in tqdm(list(data_root.glob("sample_*")), desc="Processing samples"):
        ...
        task2matrix[instruction].append(metrics)
    ...
    for instruction in instructions:
        avg_ssim = np.mean([m["SSIM"] for m in task2matrix[instruction]])
        avg_psnr = np.mean([m["PSNR"] for m in task2matrix[instruction]])
        print(f"\n{instruction}")
        print(f"Average SSIM: {avg_ssim:.4f}")
        print(f"Average PSNR: {avg_psnr:.2f} dB")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", type=str, required=True, help="ckpt dir")
    parser.add_argument("--config_path", type=str, required=True, help="train.yaml dir")

    parser.add_argument("--output_dir", type=str, default="predictions", help="output dir")
    parser.add_argument("--vae_ckpt", type=str, default=None, help="VAE checkpoint")
    parser.add_argument("--steps", type=int, default=50, help="diffusion sample step")
    parser.add_argument("--guidance_scale", type=float, default=7.5, help="guidance proportion")
